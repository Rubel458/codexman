...
export async function GET() {
  ...
  const [services, projects, pages, blogPosts, downloads] = await withTimeout(
    Promise.all([
      getServices(),
      getProjects(),
      getPublishedPages(),
    ...
    [defaultServices, defaultProjects, defaultPages, [], []] as [
      any[],
      any[],
      any[],
      any[],
  ...
  const routes: Array<{ route: string; updatedAt?: unknown }> = [
    ...
    { route: "/contact-us" },
    ...pages
      .filter((page: any) => !direct.has(page.slug))
      .map((page: any) => ({
